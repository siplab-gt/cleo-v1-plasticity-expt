...
def plot_alltuningcurves(tunings_initial,tuning_rewardend,tuning_after_rewardend,tunings_final, N_neurons,N_pop,save=None,name = ''):
        ...
        for k in range(0,N_neurons,int(groupsize)):    
            if N_pop > 1:
                plt.subplot(int(N_pop/2),int(N_pop/2),int((k/groupsize)+1))
...
def plot_grandavgtuningcurves(tunings_initial,tunings_final,N_pop,save,name = ''):
        ...
        for k in range(0,N_pop):    
            if N_pop > 1:
                plt.subplot(int(N_pop/2),int(N_pop/2),int(k+1))
...
#removed section stripped
        axa4.errorbar(ind, W_sst_pv_means[i,:]*10**9, yerr=W_sst_pv_std[i,:]*10**9, color='.75', ls = ' ', marker='o', capsize=5, capthick=1, ecolor='black')
...
#stripped
        axa6.errorbar(ind, W_sst_pv_means_afterreward[:]*10**9, yerr=W_sst_pv_std_afterreward[:]*10**9, color='.75', ls = ' ', marker='o', capsize=5, capthick=1, ecolor='black')
...
#stripped
    ...
    spiketimes=results['spike_times'][:]
    axrs1.scatter(results['spike_indice_times'][:],results['spike_indices'][:],marker='.')
    axrs1.set_xlabel('spike time')
    #axrs1.set_xlim(0,max(results['spike_times'][:]))
    axrs1.set_ylabel('spike index')
    #axrs1.set_ylim(0,max(results['spike_indices'][:]))
    plt.savefig('%s/%s/%s/recorded_spikes.pdf'%(savepath,dataname,run_no), bbox_inches='tight',format='pdf', transparent=True) 
    
    fig, axrs2 = plt.subplots(1,1)
    spike_values=results['spike_values'][:]
    #spike_values[0]=0
    axrs2.plot(results['spike_times'][:],spike_values,linewidth=.05)
    axrs2.set_xlabel('spike time')
    #axrs1.set_xlim(0,max(results['spike_times'][:]))
    axrs2.set_ylabel('spike count')
    #axrs1.set_ylim(0,max(results['spike_indices'][:]))
    plt.savefig('%s/%s/%s/recorded_spike_counts.pdf'%(savepath,dataname,run_no), bbox_inches='tight',format='pdf', transparent=True) 

    fig, axrs3 = plt.subplots(1,1)
    axrs3.plot(results['spike_times'][:],results['opto_values'][:],linewidth=.05)
    opto_intensity=results['opto_values'][:]
    #print(results['opto_values'][:])
    #print(np.shape(results['opto_values'][:]))
    axrs3.set_xlabel('time')
    #axrs1.set_xlim(0,max(results['spike_times'][:]))
    axrs3.set_ylabel('opto intensity')
    #axrs1.set_ylim(0,max(results['spike_indices'][:]))
    plt.savefig('%s/%s/%s/recorded_opto_intensity.pdf'%(savepath,dataname,run_no), bbox_inches='tight',format='pdf', transparent=True) 
    opto_intensity_sum=0
    for t_index in range(45000,65000):
        opto_intensity_sum=opto_intensity_sum+opto_intensity[t_index]
    opto_intensity_sum=opto_intensity_sum/1000
    print('Opto Power')
    print(opto_intensity_sum)

    fig, axrs4 = plt.subplots(1,1)
    firing_rate_history=results['firing_rates'][:]
    firing_rate_history[0]=0
    firing_rate_history[1]=0
    firing_rate_history[3]=0
    firing_rate_history[5]=0
    axrs4.plot(results['spike_times'][:],firing_rate_history,linewidth=.05)
    #print(results['opto_values'][:])
    #print(np.shape(results['opto_values'][:]))
    axrs4.set_xlabel('time')
    #axrs1.set_xlim(0,max(results['spike_times'][:]))
    axrs4.set_ylabel('firing rate (Hz)')
    #axrs1.set_ylim(0,max(results['spike_indices'][:]))
    plt.savefig('%s/%s/%s/recorded_firing_rates.pdf'%(savepath,dataname,run_no), bbox_inches='tight',format='pdf', transparent=True)
    ...
    ax.errorbar(ind, np.mean(W_sst_pv_means,0)*10**9, yerr=np.mean(W_sst_pv_std,0)*10**9, color='0.75', ls = ' ', marker='o', capsize=5, capthick=1, ecolor='black')
